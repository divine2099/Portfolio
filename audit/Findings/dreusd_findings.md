...
dreUSD is a USDC-backed stablecoin with an NFT-based withdrawal queue. My finding is a value-extraction that lives entirely *inside* the protocol's own circuit breaker. The oracle's deviation band is meant to protect the protocol from over-disbursing during a depeg, and at the shallow end it does the opposite. I also reconciled all 48 of my candidate findings against the two prior audits (Spearbit and Quantstamp), to separate real residual risk from what was already fixed or acknowledged. I submitted the depeg finding. It was ruled a duplicate and "not a vuln," and I escalated.
